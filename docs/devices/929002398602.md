...
| Hue | Single | Switch between scenes | The device keeps track of each press, alternating between two scenes (`"sceneid":1` and `"sceneid":2`) in a predetermined group with `"groupid":43632`, using the `commandRecall` command. Each device has an hard-coded group id that must be used in zigbee2mqtt group definition. To find id, read zigbee2mqtt logs in debug mode and search for `Received Zigbee message from 'hue_dimmer', type 'commandRecall'[...] from endpoint 1 with groupID 43632` when pressing the hue button. |
